...
description: "Create a production-ready Wes Anderson-style short drama in Lumina Canvas Agent from a story, script, or visual references. Use for multilingual canvas-native production, a first-shot seed whose duration comes from a user-approved master storyboard, an approval gate before the second video, true sequential extension to the requested duration, GBH color design, strict orthogonal-axis prompts, continuity verification, and final delivery."
---

# Lumina Wes Anderson Short Film

Produce a Wes Anderson-style short drama at the duration explicitly requested by the user in the selected or currently available Lumina node canvas. Turn story ideas, scripts, and visual references into an approved production graph: persistent specification and master-storyboard nodes, reusable character/scene/prop assets, a first-shot seed video with storyboard-defined duration, an approval gate before video two, sequential true-extension nodes, connected audio assets, and verified final delivery.
...
7. Build and obtain user approval for the master storyboard table before any video generation. Each row defines `storyboard_shot_id`, `shot_duration_seconds`, framing, camera grammar, dependency versions, story action, audio, environment/props, and transition/end state. Its duration sum must equal `target_duration_seconds`. Derive the video-step ledger one-to-one from this approved table; do not invent or optimize video durations separately.
8. Generate only `VIDEO-SEED` from the first row of the approved master storyboard. Its requested duration and prompt end time must equal that row's `shot_duration_seconds` exactly, with a local timeline beginning at `00:00`. “Short seed” describes its review role, not permission to choose a shorter duration. Verify that it plays, then show it to the user and stop. Do not create or invoke the second video (`VIDEO-EXT-01`, mapped to storyboard row 2) until the user explicitly approves `VIDEO-SEED`. If revision is requested, regenerate only the seed and present the same gate again.
9. After seed approval, generate the second video and later extensions strictly one at a time in master-storyboard order. For storyboard row `k >= 2`, `VIDEO-EXT-(k-1)` has its own independent prompt starting at `00:00`, uses that row's `shot_duration_seconds` as `added_duration_seconds`, receives the latest verified complete video, describes only that row's new beat, and must return a longer complete video. Verify the duration increase before using that output as the next input. Continue until the verified duration equals the user's locked target. Then apply [editing and compositing](references/08-editing-compositing.md) only for single-video audio finishing or export; never concatenate independent video clips to simulate extension.

Dependencies: color depends on analysis and output settings; `SPEC` depends on the direct-prompt duration and approved color; the locked screenplay and production ledger depend on `SPEC`; reusable assets depend on approved profiles; dialogue and narration depend on approved voices; `VIDEO-SEED` depends on the user-approved master storyboard row 1 and its inputs; video two depends on explicit approval of `VIDEO-SEED` plus master-storyboard row 2; every later extension depends on the immediately preceding verified complete video and the next approved storyboard row; final delivery depends on a verified complete-video lineage reaching the locked target.
...
- **Storyboard-defined video durations:** `target_duration_seconds` comes only from the user's direct prompt, while every video action's duration comes from the previously user-approved master storyboard. `VIDEO-SEED` equals row 1 `shot_duration_seconds`; for row `k >= 2`, `VIDEO-EXT-(k-1).added_duration_seconds` equals that row's `shot_duration_seconds`. The row sum must equal the final target. Never select a shorter seed or redistribute durations without revising and reapproving the master storyboard.
...
- The approved `SPEC-vN` records the target duration from the user's direct prompt, and `STORY-vN` contains the user-approved master storyboard plus a one-to-one seed/extension mapping whose row durations reconcile to it.
- `VIDEO-SEED` request duration and local prompt end time equal the approved first storyboard row's `shot_duration_seconds` exactly.
- `VIDEO-SEED` exists, plays, and has explicit user approval recorded before any extension.
- No second video node or extension action exists before that explicit first-video approval.
